...
module ODisk
  ...
  class Crypter < ::Opee::Actor
    ...
    def encrypt(src, dest, remote)
      ...
      unless $dry_run
        `gpg -c --batch --yes --force-mdc --passphrase-file --no-use-agent "#{$remote.pass_file}" -o "#{dest}" "#{src}"`
    ...
    def decrypt(src, dest)
      ...
      unless $dry_run
        `gpg -d --batch --yes -q --passphrase-file --no-use-agent "#{$remote.pass_file}" -o "#{dest}" "#{src}"`
        ::File.delete(src)
